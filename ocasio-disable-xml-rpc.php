<?php
/**
 * Plugin Name: Ocasio Disable XML-RPC
 * Plugin URI:  https://kevinocasio.com/wordpress-plugins/ocasio-disable-xml-rpc/
 * Description: Blocks the XML-RPC endpoint to protect your website against brute-force password attacks and pingback exploits.
 * Version:     1.0.0
 * Author:      Kevin Ocasio
 * Author URI:  https://kevinocasio.com
 * License:     GPL-2.0-or-later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ocasio-disable-xml-rpc
 */
...
define('OCASIO_DX_VERSION', '1.0.0');
define('OCASIO_DX_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('OCASIO_DX_PLUGIN_URL', plugin_dir_url(__FILE__));

/**
 * Helper: Smart Brand URL Resolver (Keeps links local on .local, points to live domain on production)
 */
function ocasio_dx_brand_url($path = '/wordpress-plugins/') {
 ...
 */
function ocasio_dx_activate() {
	if (get_option('ocasio_dx_enabled') === false) {
		$legacy = get_option('ko_dx_enabled');
		if ($legacy === false) {
			$legacy = get_option('ko_dxml_enabled', 1);
		}
		update_option('ocasio_dx_enabled', (int) $legacy);
	}
}
register_activation_hook(__FILE__, 'ocasio_dx_activate');

/**
 * 2. Core Logic: Disable XML-RPC Endpoint & Pingback Headers
 */
function ocasio_dx_apply_security() {
	$enabled = get_option('ocasio_dx_enabled');
	if ($enabled === false) {
		$enabled = get_option('ko_dx_enabled');
	}
	if ($enabled === false) {
		$enabled = get_option('ko_dxml_enabled', 1);
	}
	...
	// Hard kill any direct XML-RPC requests
	...
	add_filter('wp_headers', 'ocasio_dx_remove_pingback_header');
}
add_action('plugins_loaded', 'ocasio_dx_apply_security');

function ocasio_dx_remove_pingback_header($headers) {
 ...
 * 3. Register Settings
 */
function ocasio_dx_register_settings() {
	register_setting('ocasio_dx_settings_group', 'ocasio_dx_enabled', array(
		'type'              => 'integer',
		'sanitize_callback' => 'absint',
		'default'           => 1,
	));
}
add_action('admin_init', 'ocasio_dx_register_settings');

/**
 * 4. Register Admin Menu under Ocasio Plugins (Position 65)
 */
function ocasio_dx_register_menu() {
	...
	add_submenu_page(
		...
		'manage_options',
		'ocasio-disable-xml-rpc',
		'ocasio_dx_render_page'
	);
}
add_action('admin_menu', 'ocasio_dx_register_menu');

/**
 * 5. Action Link on Plugins Screen
 */
function ocasio_dx_action_links($links) {
	$settings_link = '<a href="' . esc_url(admin_url('admin.php?page=ocasio-disable-xml-rpc')) . '">' . esc_html__('Settings', 'ocasio-disable-xml-rpc') . '</a>';
	array_unshift($links, $settings_link);
	return $links;
}
add_filter('plugin_action_links_' . plugin_basename(__FILE__), 'ocasio_dx_action_links');

/**
 * 6. Conditional Admin Asset Enqueuing
 */
function ocasio_dx_admin_assets($hook) {
	...
	if (strpos($hook, 'ocasio-disable-xml-rpc') !== false || strpos($hook, 'ocasio-plugins-main') !== false || $hook === 'toplevel_page_ocasio-plugins-main') {
		wp_enqueue_style('ocasio-dx-admin-css', plugins_url('assets/admin.css', __FILE__), array(), OCASIO_DX_VERSION);
		wp_enqueue_script('ocasio-dx-admin-js', plugins_url('assets/admin.js', __FILE__), array('jquery'), OCASIO_DX_VERSION, true);
		wp_localize_script('ocasio-dx-admin-js', 'ocasio_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
		wp_localize_script('ocasio-dx-admin-js', 'ocasio_301_vars', array(
			'ajaxurl'     => admin_url('admin-ajax.php'),
			'suite_nonce' => wp_create_nonce('ocasio_suite_toggle_nonce'),
		));
	}
}
add_action('admin_enqueue_scripts', 'ocasio_dx_admin_assets');

/**
 * 7. Dedicated Settings Page (620px Centered Card)
 */
function ocasio_dx_render_page() {
	if (!current_user_can('manage_options')) {
		return;
	}

	$is_enabled = get_option('ocasio_dx_enabled');
	if ($is_enabled === false) {
		$is_enabled = get_option('ko_dx_enabled');
	}
	if ($is_enabled === false) {
		$is_enabled = get_option('ko_dxml_enabled', 1);
	}
	$is_enabled = ((int) $is_enabled === 1);

	$author_url = ocasio_dx_brand_url('/');
	$hub_url    = ocasio_dx_brand_url('/wordpress-plugins/');
	?>
		...
		<div class="ko-plugin-container">
			...
			<div class="ko-plugin-header">
				<h1 class="ko-plugin-header-title">
					...
					<span class="ko-title-text">DISABLE XML-RPC</span>
				</h1>
			</div>

			<!-- Body Stage -->
			<div class="ko-plugin-body">
				<p class="ko-plugin-intro">Blocks the XML-RPC endpoint to protect your website against automated brute-force password guessing and pingback exploits.</p>

				<form method="post" action="options.php">
					<?php settings_fields('ocasio_dx_settings_group'); ?>

					<div class="ko-setting-box">
						<!-- Row 1: Disable XML-RPC -->
						<div class="ko-setting-row">
							<div class="ko-setting-info">
								<strong>Block XML-RPC API</strong>
								<p>Immediately kills access to <code>xmlrpc.php</code> and drops pingback headers.</p>
							</div>
							<label class="ko-switch">
								<input type="hidden" name="ocasio_dx_enabled" value="0">
								<input type="checkbox" name="ocasio_dx_enabled" value="1" <?php checked($is_enabled, true); ?>>
					...
					<div class="ko-meta-section" style="margin-top:20px;">
						<?php if ($is_enabled): ?>
							<div style="background:#f0fdf4; border:1px solid #bbf7d0; color:#166534; padding:12px 16px; border-radius:6px; font-size:13px; font-weight:500;">
								<strong style="color:#15803d;">Status: Protected</strong> &mdash; XML-RPC is blocked. Malicious requests receive an immediate 403 Forbidden stop sign.
							</div>
						<?php else: ?>
							<div style="background:#fef2f2; border:1px solid #fecaca; color:#991b1b; padding:12px 16px; border-radius:6px; font-size:13px; font-weight:500;">
								<strong style="color:#b91c1c;">Status: Open</strong> &mdash; XML-RPC is active. Bots can test passwords and query pingbacks through <code>xmlrpc.php</code>.
							</div>
						<?php endif; ?>
					</div>
